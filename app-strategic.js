...
class Resource {
    constructor(id, name, type = 'person') {
        ...
        // Unit of measurement
        this.unit = this.getDefaultUnit(); // 'hours', 'dollars', 'units'
        this.capacityPeriod = 'daily'; // 'daily', 'monthly', 'quarterly', 'annual' (for budget resources)

        // Capacity settings
        this.capacity = 1; // Number of units (e.g., 3 FTEs)
        this.availabilityPercent = 80; // 80% availability
        this.maxCapacityPerDay = 8; // hours per day (for people) or dollars per period (for budget)
    ...
    getCapacityLabel() {
        if (this.unit === 'dollars') {
            ...
            return `$${this.maxCapacityPerDay.toLocaleString()}${periodLabels[this.capacityPeriod] || ''}`;
        } else if (this.unit === 'hours') {
            return `${this.maxCapacityPerDay} hrs/day`;
        } else {
            return `${this.maxCapacityPerDay} ${this.unit}/day`;
        }
    }
    ...
    toString() {
        return `${this.getTypeIcon()} ${this.name} (${this.getCapacityLabel()} @ ${this.availabilityPercent}%)`;
    }
...
class SystemicProjectState {
    ...
    loadCommonResources() {
        const templates = [
            { name: 'Owner / CEO', type: 'decision_maker', role: 'Owner', capacity: 1, availability: 60, loadLimit: 70, maxPerDay: 8, isDrum: true },
            { name: 'Investment Committee', type: 'decision_maker', role: 'Committee', capacity: 1, availability: 40, loadLimit: 60, maxPerDay: 8 },
            { name: 'Fractional CFO', type: 'person', role: 'CFO', capacity: 1, availability: 50, loadLimit: 75, maxPerDay: 8, isShared: true },
            { name: 'Exit Planner', type: 'person', role: 'Planner', capacity: 1, availability: 80, loadLimit: 75, maxPerDay: 8, isShared: true },
            { name: 'Designer / Engineer', type: 'skill_pool', role: 'Designer', capacity: 2, availability: 80, loadLimit: 75, maxPerDay: 8 },
            { name: 'SME (Subject Matter Expert)', type: 'skill_pool', role: 'SME', capacity: 1, availability: 70, loadLimit: 70, maxPerDay: 8 },
            { name: 'Operator', type: 'skill_pool', role: 'Operator', capacity: 3, availability: 80, loadLimit: 80, maxPerDay: 8 },
            { name: 'Conference Room A', type: 'space', location: 'Main Building', capacity: 1, maxPerDay: 1, unit: 'units' },
            { name: 'Workshop Bay 1', type: 'space', location: 'Workshop', capacity: 1, maxPerDay: 1, unit: 'units' },
            { name: 'Monthly OpEx Budget', type: 'financial', role: 'Operating Budget', capacity: 1, availability: 80, loadLimit: 75, maxPerDay: 50000, unit: 'dollars', period: 'monthly' },
            { name: 'Q1 Capital Budget', type: 'financial', role: 'Capital Projects', capacity: 1, availability: 100, loadLimit: 80, maxPerDay: 150000, unit: 'dollars', period: 'quarterly' }
        ];

        templates.forEach(t => {
            const resource = this.addResource(t.name, t.type);

            // Set unit first (important for proper initialization)
            if (t.unit) {
                resource.unit = t.unit;
            }
            if (t.period) {
                resource.capacityPeriod = t.period;
            }

            // Then set other properties
            resource.role = t.role || '';
            resource.capacity = t.capacity || 1;
            resource.availabilityPercent = t.availability || 80;
            resource.loadLimitPercent = t.loadLimit || 75;
...
function createTaskElement(task, objectiveId, tacticId, initiativeId) {
    ...
    if (task.resourceAssignments && task.resourceAssignments.length > 0) {
        task.resourceAssignments.forEach(assignment => {
            ...
            if (resource) {
                const unitDisplay = resource.unit === 'dollars' ? `$${assignment.hoursPerDay}/d` :
                                   resource.unit === 'hours' ? `${assignment.hoursPerDay}h/d` :
                                   `${assignment.hoursPerDay}${resource.getUnitLabel()}/d`;
                resourceBadges += `<span style="background: #e0e0e0; padding: 2px 6px; border-radius: 3px; font-size: 10px; margin-right: 4px;">
                    ${resource.getTypeIcon()} ${resource.name} (${unitDisplay})
                </span>`;
            }
        });
    }
...
function showAddTaskForm(objectiveId, tacticId, initiativeId) {
    ...
    if (systemicState.resources.length === 0) {
        ...
        systemicState.resources.forEach(r => {
            const unitLabel = r.getUnitLabel();
            const placeholder = r.unit === 'dollars' ? '$/day' :
                               r.unit === 'hours' ? 'hrs/day' : `${unitLabel}/day`;
            const defaultValue = r.unit === 'dollars' ? '100' :
                                r.unit === 'hours' ? '8' : '1';

            resourceCheckboxes += `
                <div style="margin-bottom: 8px; padding: 6px; background: #f9f9f9; border-radius: 4px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" class="resource-checkbox" value="${r.id}" data-name="${r.name}" data-unit="${r.unit}">
                        <span>${r.getTypeIcon()} ${r.name}</span>
                        <input type="number" class="resource-hours" data-resource="${r.id}"
                               placeholder="${placeholder}" min="0.01" step="0.01" value="${defaultValue}"
                               style="width: 80px; margin-left: auto; padding: 4px; font-size: 12px;"
                               disabled>
                        <span style="font-size: 10px; color: #666; min-width: 40px;">${placeholder}</span>
                    </label>
                </div>
            `;
        });
    ...
    content.innerHTML = `
        ...
        <div class="form-group">
            ...
            <p style="font-size: 11px; color: #666; margin-top: 5px;">
                💡 Check resources and set amount per day for each. Tasks can use multiple resources simultaneously.
            </p>
        </div>
        <div class="form-actions">
...
function editTask(objectiveId, tacticId, initiativeId, taskId) {
    ...
    if (systemicState.resources.length === 0) {
        ...
        systemicState.resources.forEach(r => {
            ...
            const defaultValue = r.unit === 'dollars' ? '100' :
                                r.unit === 'hours' ? '8' : '1';
            const hoursValue = assignment ? assignment.hoursPerDay : defaultValue;
            const hoursDisabled = assignment ? '' : 'disabled';

            resourceCheckboxes += `
                <div style="margin-bottom: 8px; padding: 6px; background: #f9f9f9; border-radius: 4px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" class="resource-checkbox" value="${r.id}" data-name="${r.name}" data-unit="${r.unit}" ${isChecked}>
                        <span>${r.getTypeIcon()} ${r.name}</span>
                        <input type="number" class="resource-hours" data-resource="${r.id}"
                               placeholder="${placeholder}" min="0.01" step="0.01" value="${hoursValue}"
                               style="width: 80px; margin-left: auto; padding: 4px; font-size: 12px;"
                               ${hoursDisabled}>
                        <span style="font-size: 10px; color: #666; min-width: 40px;">${placeholder}</span>
                    </label>
...
function showAddResourceForm() {
    ...
    content.innerHTML = `
        ...
        <div class="form-group">
            <label>Type:</label>
            <select id="resourceType" onchange="updateResourceFormFields()">
        ...
        <div class="form-group">
            ...
            <select id="resourceUnit" onchange="updateResourceFormFields()">
        ...
        <div class="form-group">
            <label id="maxPerDayLabel">Max Hours/Day:</label>
            <input type="number" id="resourceMaxPerDay" min="1" value="8" step="0.01">
            <small id="maxPerDayHelp" style="color: #666; display: block; margin-top: 4px;">
                For people: typical work hours per day
            </small>
        </div>
        <div class="form-group">
    ...
    modal.style.display = 'block';
    updateResourceFormFields(); // Initialize form state
}

function updateResourceFormFields() {
    const type = document.getElementById('resourceType')?.value;
    const unit = document.getElementById('resourceUnit');
    const periodGroup = document.getElementById('periodGroup');
    const maxPerDayLabel = document.getElementById('maxPerDayLabel');
    const maxPerDayInput = document.getElementById('resourceMaxPerDay');
    const maxPerDayHelp = document.getElementById('maxPerDayHelp');
    ...
    if (type === 'financial') {
        ...
        maxPerDayLabel.textContent = 'Budget Amount:';
        maxPerDayInput.step = '0.01';
        maxPerDayInput.value = '10000';
        maxPerDayHelp.textContent = 'Total budget available per period';
    } else if (type === 'space') {
        unit.value = 'units';
        periodGroup.style.display = 'none';
        maxPerDayLabel.textContent = 'Max Units/Day:';
        maxPerDayInput.step = '1';
        maxPerDayInput.value = '1';
        maxPerDayHelp.textContent = 'Maximum units available (e.g., seats, square feet)';
    } else {
        unit.value = 'hours';
        periodGroup.style.display = 'none';
        maxPerDayLabel.textContent = 'Max Hours/Day:';
        maxPerDayInput.step = '0.5';
        maxPerDayInput.value = '8';
        maxPerDayHelp.textContent = 'For people: typical work hours per day';
    }

    // Show/hide period selector for dollars
    if (unit.value === 'dollars') {
        periodGroup.style.display = 'block';
    } else {
        periodGroup.style.display = 'none';
    }
...
function saveResource() {
    ...
    resource.unit = document.getElementById('resourceUnit').value;
    resource.capacityPeriod = document.getElementById('resourcePeriod')?.value || 'daily';
    ...
    resource.maxCapacityPerDay = parseFloat(document.getElementById('resourceMaxPerDay').value);
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <div style="flex:1;">
                        ...
                        <div style="font-size:10px;color:#666;margin-top:2px;">
                            ${r.getCapacityLabel()} @ ${r.availabilityPercent}% avail. × ${r.loadLimitPercent}% load limit
                            ${r.role ? ' | ' + r.role : ''}
                        </div>
                    </div>
...
function drawResourceLoad() {
    ...
    resourceData.forEach((data, idx) => {
        ...
        ctx.fillText(label, leftMargin - 10, y + barHeight / 2 - 2);

        // Capacity sublabel
        ctx.fillStyle = '#666';
        ctx.font = '10px Arial';
        ctx.fillText(`(${data.capacityLabel})`, leftMargin - 10, y + barHeight / 2 + 10);
